...
public class NewsContentHelper
{
    ...
    public NewsContentHelper(IContentTypeService contentTypeService)
    {
        ...
        string culture)
    {
        ...
        pageContent.SetValue<PageNews>(p => p.Date, GetDateOrNull((string?) post.Element("pubDate")), culture);
        ...
        {
            pageContent.SetValue<PageNews>(p => p.ContentTags, JsonSerializer.Serialize(new[] { category.Value }), culture);
        }
